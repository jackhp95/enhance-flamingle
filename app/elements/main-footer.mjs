export default function mainFooter({ html, state }) {
  return html`<footer
    class="will-change-[max-width] sm:int:max-w-max sm:max-w-min font-light sm:h-screen sm:w-auto sm:flex-col group sticky inset-0 w-full flex justify-between overflow-x-auto overflow-y-hidden sm:overflow-hidden bg-black bg-opacity-75 backdrop-blur ring-1 ring-fg fg-a-5"
  >
    <div class="contents fg-a-100">
      <nav class="sticky top-0 flex sm:flex-col flex-none" aria-label="Sidebar">
        <a
          href="/"
          class="p-4 sm:px-3 sm:py-5 sm:my-8 flex h-16 gap-x-0 sm:group-int:gap-x-1 items-center text-fg opacity-60 int:opacity-100 int:text-act sm:origin-left int:scale-105 active:scale-95 transition-all ease-out"
        >
            <span
              class="text-3xl sm:text-4xl transition-all ease-out opacity-60 flex-shrink-0 flex justify-center icon logo"
            ></span>
            <span
              class="sm:group-int:tracking-normal tracking-[-0.25ch] font-bold sm:group-int:text-lg text-xs sm:group-int:max-w-[max(10ch,10vw)] max-w-0 overflow-hidden transition-all ease-out h-[1.5em] flex gap-x-0 items-center flex-wrap gap-y-64"
            >
        ...
        <a
          href="/events/"
          class="p-5 flex h-16 gap-x-0 sm:group-int:gap-x-4 items-center text-fg opacity-60 int:opacity-100 int:text-act sm:origin-left int:scale-105 active:scale-95 transition-all ease-out"
        >
          <span
      ...
      <a
        href="/account"
        class="p-5 flex h-16 gap-x-0 sm:group-int:gap-x-4 items-center text-fg transition-all ease-out opacity-60 int:opacity-100 int:text-act sm:origin-left"
      >
        <span
